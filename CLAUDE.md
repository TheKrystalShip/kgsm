...
- **Blueprints** (`blueprints/native/*.bp` and `blueprints/container/*.docker-compose.yml`) are templates declaring server parameters: `name`, `executable_file`, `level_name`, `ports`, optional `steam_app_id`, etc. `ports` format is single-quoted, pipe-separated: `'1111:2222/tcp|3333/udp'`. These are the system (read-only) blueprints; user blueprints live under `~/.local/share/kgsm/blueprints/{native,container}/` and shadow same-named system ones. See `blueprints/CLAUDE.md`.
...
**Critical gotcha:** a blueprint binds to an override directory by its logical *name*, *not* its filename — the `name=` field for native blueprints, the first `services:` entry for container blueprints. `terraria-modded.bp` with `name=terraria` uses `overrides/terraria/`. This lets multiple blueprint variants share one override directory.
...
- Examples: `blueprints/default/native/`, `overrides/factorio.overrides.sh`, `migrations/config/001_v0_to_v1_flat_to_sectioned.sh`
