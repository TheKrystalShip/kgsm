...
# KGSM Files Symlink Command CLI Tests
#
# Test Type: UNIT
# Target: commands/files.symlink.sh - CLI interface and argument handling
#
# Tests the CLI interface of files.symlink.sh including help system,
...
readonly TEST_NAME="files_symlink_commands"
readonly MODULE="$KGSM_ROOT/commands/files.symlink.sh"
...
function setup_test() {
  log_test_step "Setting up files.symlink commands tests"
  ...
  assert_file_exists "$MODULE" "files.symlink.sh module should exist"
  assert_file_executable "$MODULE" "files.symlink.sh should be executable"
...
function test_disable_on_fresh_instance() {
  log_test_step "Testing disable on fresh instance with no symlink configured"
  ...
  # A fresh instance has no command_shortcut_file configured,
  # so disable should succeed (no-op path)
  local output
  output=$("$MODULE" disable "$instance_name" 2>&1)
  local exit_code=$?

  assert_equals 0 "$exit_code" "disable on fresh instance (no symlink configured) should succeed"
...
function main() {
  log_test_step "Starting files.symlink commands tests"
  ...
  log_test_step "Files.symlink commands tests completed"

  if print_assert_summary "$TEST_NAME"; then
    pass_test "All files.symlink commands tests passed"
  else
    fail_test "Some files.symlink commands tests failed"
  fi
}
