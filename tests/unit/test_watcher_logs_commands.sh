...
function main() {
  log_test_step "Starting watcher logs commands tests"
  ...
  test_status_shows_pattern_info

  # Test command error paths
  test_test_on_instance_without_log_pattern

  log_test_step "Watcher logs commands tests completed"

  if print_assert_summary "$TEST_NAME"; then
    pass_test "All watcher logs commands tests passed"
  else
    fail_test "Some watcher logs commands tests failed"
  fi
}
