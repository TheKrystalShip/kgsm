...
function main() {
  log_test_step "Starting files.systemd commands tests"
  ...
  # Systemd operations (conditional on system capabilities)
  test_enable_on_valid_instance_requires_systemd
  test_disable_on_valid_instance_no_systemd_files

  log_test_step "files.systemd commands tests completed"

  if print_assert_summary "$TEST_NAME"; then
    pass_test "All files.systemd commands tests passed"
  else
    fail_test "Some files.systemd commands tests failed"
  fi
}
