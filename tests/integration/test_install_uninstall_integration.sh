...
function main() {
  log_test_step "Starting install+uninstall integration tests"

  setup_test

  # install.sh error case tests
  test_install_fails_with_nonexistent_blueprint
  test_install_fails_with_missing_blueprint
  test_install_fails_with_missing_install_dir_value

  # install.sh partial workflow tests
  test_install_creates_instance_config_before_download
  test_install_with_name_creates_named_instance_dir
  test_install_creates_directory_structure

  # uninstall.sh error case tests
  test_uninstall_fails_with_nonexistent_instance
  test_uninstall_cancels_on_no_answer

  # uninstall.sh workflow tests
  test_uninstall_removes_instance_config
  test_uninstall_removes_instance_symlink
  test_uninstall_removes_working_directory
  test_uninstall_removes_from_instances_list

  # Full cycle tests
  test_full_cycle_create_files_uninstall
  test_uninstall_does_not_affect_sibling_instances

  log_test_step "Install+uninstall integration tests completed"

  if print_assert_summary "$TEST_NAME"; then
    pass_test "All install+uninstall integration tests passed"
  else
    fail_test "Some install+uninstall integration tests failed"
  fi
}
