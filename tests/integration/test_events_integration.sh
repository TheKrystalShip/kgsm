...
function main() {
  log_test_step "Starting events integration tests"

  setup_test

  # Status and delegation tests
  test_status_aggregates_both_transports
  test_socket_delegation_status
  test_webhook_delegation_status

  # Test command routing tests
  test_test_all_fails_with_no_transports
  test_test_invalid_transport_fails
  test_test_missing_transport_fails
  test_test_socket_routes_to_socket_module
  test_test_webhook_routes_to_webhook_module

  # Emit behavior tests
  test_emit_with_broadcasting_disabled_returns_success
  test_emit_no_arguments_fails
  test_emit_invalid_event_when_enabled_fails
  test_emit_valid_event_no_transports_fails

  # Transport emit behavior
  test_socket_emit_graceful_with_no_socket_file
  test_webhook_emit_fails_with_no_urls

  # Default status tests
  test_socket_status_shows_disabled_by_default
  test_webhook_status_shows_disabled_by_default

  # Enable/disable command tests
  test_socket_disable_command
  test_webhook_disable_command
  test_socket_enable_disable_cycle
  test_webhook_enable_disable_cycle

  # Help delegation tests
  test_events_socket_help_delegation
  test_events_webhook_help_delegation

  # Path tests
  test_socket_file_path_within_kgsm_root

  log_test_step "Events integration tests completed"

  if print_assert_summary "$TEST_NAME"; then
    pass_test "All events integration tests completed successfully"
  else
    fail_test "Some events integration tests failed"
  fi
}
