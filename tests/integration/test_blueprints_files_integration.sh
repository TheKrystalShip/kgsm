...
function main() {
  log_test_step "Starting blueprints files integration tests"

  setup_test

  # Native blueprint tests
  test_native_list_contains_known_blueprints
  test_native_list_default_filter
  test_native_list_custom_filter
  test_native_find_returns_bp_extension
  test_native_info_contains_required_fields
  test_native_info_json_format
  test_native_list_json_format
  test_native_find_invalid_blueprint_fails
  test_native_info_invalid_blueprint_fails
  test_native_find_consistent_with_blueprints_sh
  test_native_find_path_enables_instance_creation
  test_native_find_different_blueprints_different_paths

  # Container blueprint tests
  test_container_list_contains_vrising
  test_container_list_default_filter
  test_container_find_returns_docker_compose_path
  test_container_info_contains_name
  test_container_info_json_format
  test_container_find_invalid_blueprint_fails

  # Isolation tests
  test_native_module_cannot_find_container_blueprint
  test_container_module_cannot_find_native_blueprint
  test_native_blueprint_instance_has_native_runtime

  log_test_step "Blueprints files integration tests completed"

  if print_assert_summary "$TEST_NAME"; then
    pass_test "All blueprints files integration tests passed"
  else
    fail_test "Some blueprints files integration tests failed"
  fi
}
