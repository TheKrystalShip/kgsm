...
function main() {
  log_test_step "Starting files+instances integration tests"

  setup_test

  # Management file tests
  test_management_file_creation
  test_management_file_content
  test_management_file_removal
  test_management_file_fails_for_nonexistent_instance

  # files.sh orchestrator tests
  test_files_orchestrator_create
  test_files_orchestrator_remove
  test_files_orchestrator_fails_for_nonexistent_instance

  # Symlink integration tests
  test_symlink_enable_creates_symlink
  test_symlink_disable_removes_symlink
  test_symlink_enable_fails_without_management_file
  test_symlink_fails_for_nonexistent_instance

  # UPnP integration tests
  test_upnp_enable_updates_config
  test_upnp_disable_updates_config
  test_upnp_fails_for_nonexistent_instance

  # Error handling for systemd/ufw
  test_systemd_and_ufw_fail_for_nonexistent_instance

  # Cross-instance and idempotency tests
  test_two_instances_have_independent_management_files
  test_files_create_is_idempotent
  test_management_file_creation_for_terraria
  test_upnp_idempotent
  test_full_files_workflow

  log_test_step "Files+instances integration tests completed"

  if print_assert_summary "$TEST_NAME"; then
    pass_test "All files+instances integration tests passed"
  else
    fail_test "Some files+instances integration tests failed"
  fi
}
