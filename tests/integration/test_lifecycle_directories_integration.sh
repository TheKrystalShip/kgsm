...
function main() {
  log_test_step "Starting lifecycle+directories integration tests"

  setup_test

  # Error behavior (no instance needed)
  test_lifecycle_fails_on_nonexistent_instance
  test_directories_errors_on_nonexistent_instance

  # Directory structure tests (instance needed)
  test_directories_create_builds_expected_structure
  test_directories_create_updates_config_paths
  test_lifecycle_is_active_returns_nonzero_for_stopped_instance
  test_directories_remove_destroys_structure
  test_lifecycle_fails_after_directories_remove
  test_lifecycle_with_stub_manage_script
  test_directories_create_is_idempotent
  test_two_instances_have_independent_directories

  # Utility tests (no instance needed)
  test_ensure_created_supports_lifecycle_paths

  # Cycle tests (instance needed)
  test_directories_create_remove_create_cycle

  log_test_step "Lifecycle+directories integration tests completed"

  if print_assert_summary "$TEST_NAME"; then
    pass_test "All lifecycle+directories integration tests passed"
  else
    fail_test "Some lifecycle+directories integration tests failed"
  fi
}
