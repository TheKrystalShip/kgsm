...
and management scripts. Full reference: `docs/templates.md`.
...
The numbered modules are concatenated in numeric order into one
...
- `03-lifecycle` `04-io` `05-version` `06-download` `07-deploy` `08-backup`
  `10-logging` `11-status` — the **overridable** modules; per-game modules in
  `overrides/<name>/` replace these by filename when present.
