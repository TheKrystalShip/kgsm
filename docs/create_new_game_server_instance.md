...
- Select a blueprint.
- Select a library to place the instance in.
...
Here's how the command `./kgsm.sh install factorio --library ssd` breaks down into its individual steps, with `/opt/test` standing in for the library `ssd`'s root:
...
```sh
./kgsm.sh libraries add /opt/test --name ssd
```

This registers the root instances are placed in and writes its `.kgsm-library`
marker. A library only has to be registered once; `./kgsm.sh libraries list`
shows the registered ones with their free space.
...
```sh
working_dir="/opt/test/instances/factorio/${instance}"
...
- **Register Your Roots**: `kgsm libraries add <path>` names a root and reports its free space — see [Libraries](libraries.md).
...
| **Issue**                                 | **Resolution**                                              |
| ----------------------------------------- | ----------------------------------------------------------- |
| No libraries registered                   | Run `./kgsm.sh libraries add <path>`.                       |
| Several libraries and none chosen         | Pass `--library <name>`, or set `default_library`.          |
| Not enough free space in the library      | Free space, choose another library, or pass `--skip-space-check`. |
