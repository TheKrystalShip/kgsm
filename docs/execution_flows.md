...
graph TD
    ...
    C --> D["files.sh remove INSTANCE<br/>(management script, systemd, UFW, UPnP, symlinks)"]
...
2. **File removal**: All generated files (management script, optional systemd/UFW/UPnP/symlink integrations) are removed by `files.sh remove`.
...
- **systemd**: Service file generation and management (`files.systemd.sh`, enabled via `config_enable_systemd`)
